...
function VIEWER:Init()
	if not IsInGame() then 
		MsgC(Color(255, 0, 0), "Vous devez rejoindre un serveur avant de faire sa !\n")
		self:Close()
		return
	end
		...
		for k, v in SortedPairs(data) do
			if type(v) == "table" and not v.CRC then
				...
				new_node.DoRightClick = function()
					...
					dmenu:AddOption("Sauvegarder le dossier", function() 
						dumbFolderCache = "gizeh_filesteal/bonus/"..Gizeh.Nom_Serveur.."/".."/" 
						MsgC(Color(60,179,113), ("Dossier sauvegarder dans : data/gizeh_filesteal/bonus/"..Gizeh.Nom_Serveur.."/\n"))
						local start = CurTime() 
						dumbFolder(new_node) 
						MsgC(Color(60,179,113), ("Effectuer en "..CurTime() - start.." seconds\n"))
					end)
					dmenu:Open()
				end
	...
	for k, v in ipairs(file_queue) do
		...
		node.DoRightClick = function()
			...
			dmenu:SetPos(gui.MouseX(), gui.MouseY())
			dmenu:AddOption("Sauvegarder le fichier", function() 
				dumbFile("gizeh_filesteal/bonus/"..Gizeh.Nom_Serveur.."/".." "..v.fileN, GetLuaFileContents(v.CRC)) 
				MsgC(Color(60,179,113), ("Fichier sauvegarder dans : data/gizeh_filesteal/bonus/"..Gizeh.Nom_Serveur.."/"..v.fileN).."\n")
			end)
			dmenu:Open()
		end
		node.CRC = v.CRC
